...
class PostManagerTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.cat_1.save()
        cls.cat_2.save()
        cls.post_1 = Post(title='title',
                          tags='tag1,tag2',
                          date=cls.time,
                          category=cls.cat_1,
                          content='test content')
        cls.post_2 = Post(title='title1',
                          tags='tag1',
    ...
    def test_get_posts_by_category_normal_cond(self):
        """
        Test get_posts_by_category in normal condition
        """
        post_list = Post.data_api.get_posts_by_category(self.cat_1.url)
        self.assertQuerysetEqual(post_list,
                                 [repr(i) for i in [self.post_1, self.post_2]])

    def test_get_posts_by_category_empty_cond(self):
        """
        Test get_posts_by_category when category is empty
        """
        post_list = Post.data_api.get_posts_by_category(self.cat_2.url)
        self.assertQuerysetEqual(post_list,
                                 [])

    def test_get_posts_by_category_limit_cond(self):
        """
        Test get_posts_by_category when limit options is applied
        """
        post_list = Post.data_api.get_posts_by_category(self.cat_1.url,
                                                        limit=1)
        self.assertQuerysetEqual(post_list,
    ...
    def test_get_posts_all_posts(self):
        ...
        self.assertQuerysetEqual(posts, [repr(i) for i in [self.post_1,
                                                           self.post_2]])
    ...
    def test_get_posts_limited_posts(self):
        ...
        self.assertQuerysetEqual(posts, [repr(self.post_1)])
